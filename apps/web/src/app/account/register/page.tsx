...
import { Loader2, Eye, EyeOff, ShoppingBag, Lock, Mail, User } from 'lucide-react';
...
const schema = z.object({
  ...
  password: z.string().min(8, 'كلمة المرور 8 أحرف على الأقل').regex(/[A-Z]/, 'يجب أن تحتوي على حرف كبير').regex(/[0-9]/, 'يجب أن تحتوي على رقم'),
});
type FormData = z.infer<typeof schema>;

function CustomerRegisterForm() {
  useDocumentTitle('إنشاء حساب عميل');
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true);
    try {
      const res = await api.post<{ success: boolean; data: AuthResponse }>('/api/auth/customer-register', data);
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 10, padding: '40px 32px', boxShadow: '0 8px 40px rgba(47,46,75,.08)' }}>
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <h1 style={{ fontFamily: 'var(--font-cairo)', fontWeight: 800, fontSize: 24, color: C.text, marginBottom: 6 }}>أنشئ حسابك</h1>
            <p style={{ fontSize: 14, color: C.muted }}>تابع طلباتك من كل متاجر بازار بحساب واحد</p>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: C.p, marginBottom: 6 }}>الاسم الكامل</label>
              <div style={{ position: 'relative' }}>
                <input {...register('name')} placeholder="أحمد محمد"
                  style={{ width: '100%', padding: '12px 40px 12px 16px', background: C.bg, border: `1.5px solid ${errors.name ? '#f87171' : C.border}`, borderRadius: 10, fontSize: 14, outline: 'none' }} />
                <User size={16} color={C.muted} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)' }} />
              </div>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input {...register('password')} type={showPw ? 'text' : 'password'} placeholder="8 أحرف على الأقل"
          ...
          <p style={{ textAlign: 'center', fontSize: 13, color: C.muted, marginTop: 24 }}>
            لديك حساب بالفعل؟{' '}
            <Link href="/account/login" style={{ color: C.a, fontWeight: 700, textDecoration: 'none' }}>سجّل الدخول</Link>
          </p>
        </div>
      </div>
    </div>
